...
void RenderShape(SDL_Renderer *renderer, RigidBody *rbody, BodyGeometry *gbody);
...
void RenderFrame(Render *render, Engine *engine) {
    ...
    for (ui32 i = 0; i < engine->active_amount; i++) {
        RigidBody *rbody = &engine->rbodies[i];
        BodyGeometry *gbody = &engine->gbodies[i];
        RenderShape(renderer, rbody, gbody);
    }
    if (engine->active_amount > 0) {
        SDL_SetRenderDrawColor(renderer, 0, 0, 0, SDL_ALPHA_OPAQUE);
        SDL_RenderDebugTextFormat(renderer, 0.0f, 0.0f, "Body1 velocity: { %f;%f }", engine->rbodies[0].velocity.x, engine->rbodies[0].velocity.y);
    }
...
void RenderShape(SDL_Renderer *renderer, RigidBody *rbody, BodyGeometry *gbody) {
    ui8 vertice_amount = gbody->data.vertice_cnt;

    if (gbody->shape == BSHAPE_CIRCLE) { vertice_amount = 65; }
    ...
    if (gbody->shape == BSHAPE_CIRCLE) {
        vertices[0].position.x = rbody->screen_pos.x;
        vertices[0].position.y = rbody->screen_pos.y;
        vertices[0].color = color;

        for (ui8 i = 1; i < vertice_amount; i++) {
            f32 radians = 2.0f * PI_F * (float)i / (float)(vertice_amount-1);
            vertices[i].position.x = vertices[0].position.x + (gbody->data.radius * PIXELS_PER_METER) * SDL_cosf(radians);
            vertices[i].position.y = vertices[0].position.y + (gbody->data.radius * PIXELS_PER_METER) * SDL_sinf(radians);
            vertices[i].color = color;
        }
    }
    else if (gbody->shape == BSHAPE_POLYGON) {
        for (ui8 i = 0; i < vertice_amount; i++) {
            vertices[i].color = color;
            vertices[i].position.x = (gbody->data.vertices[i].x * PIXELS_PER_METER) + rbody->screen_pos.x;
            vertices[i].position.y = (gbody->data.vertices[i].y * PIXELS_PER_METER) + rbody->screen_pos.y;
        }
    }
    ...
    if (gbody->shape == BSHAPE_CIRCLE) { indices[index_amount-1] = 1; } // lock the last triangle
